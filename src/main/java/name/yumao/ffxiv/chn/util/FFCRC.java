...
public class FFCRC {
	...
	public static int ComputeCRC(byte[] ecxBytes) {
		return ComputeCRC(ecxBytes, 0, ecxBytes.length);
	}

	public static int ComputeCRC(byte[] ecxBytes, int offset) {
		return ComputeCRC(ecxBytes, offset, ecxBytes.length);
	}

	public static int ComputeCRC(byte[] ecxBytes, int offset, int cbLength) {
		ByteBuffer pbBuffer = ByteBuffer.wrap(ecxBytes, offset, cbLength);
	...
	public static void main(String[] vargs) {
		...
		int crc = ComputeCRC(file.toLowerCase().getBytes());
	 ...
	 */
	@Deprecated
	public static int _ComputeCRC(byte[] ecxBytes, int offset, int edxLength) {
		ByteBuffer ecxBB = ByteBuffer.wrap(ecxBytes, offset, edxLength);
		ecxBB.order(ByteOrder.LITTLE_ENDIAN);
		int eax = -1;
		int esiLength = edxLength;
		if (esiLength != 0) {
			//something about adress, skipping
		}
		int edi = 0;
		int edx = 0;
		int ebx = 0;
		...
		if (esiLength >= 0x20) {
			...
			do {
				...
				ebx &= xff;
				edx &= xff;
				edx = crc_table_0f089d0[edx];
				edx ^= crc_table_0f08dd0[ebx];
				ebx = eax;
				ebx >>>= 0x18;
				edx ^= crc_table_0f085d0[ebx];
				eax &= xff;
				edx ^= crc_table_0f091d0[eax];

				edx ^= ecxBB.getInt();
				eax = edx;
				eax >>>= 0x10;
				eax &= xff;
				eax = crc_table_0f089d0[eax];
				ebx = edx;
				ebx >>>= 0x8;
				ebx &= xff;
				eax ^= crc_table_0f08dd0[ebx];
				ebx = edx;
				ebx >>>= 0x18;
				eax ^= crc_table_0f085d0[ebx];
				edx &= xff;
				eax ^= crc_table_0f091d0[edx];
		...
		if (esiLength >= 4) {
			...
			do {
				...
				edi >>>= 0x10;
				edi &= xff;
				edi = crc_table_0f089d0[edi];
				ebx = eax;
				ebx >>>= 0x8;
				ebx &= xff;
				edi ^= crc_table_0f08dd0[ebx];
				ebx = eax;
				ebx >>>= 0x18;
				edi ^= crc_table_0f085d0[ebx];
				eax &= xff;
				edi ^= crc_table_0f091d0[eax];
				esiLength -= 4;
				edx--;
				eax = edi;
			} while (edx > 0);
