...
public class SqPackIndex {
	...
	public HashMap<Integer, SqPackIndexFolder> resloveIndex() throws Exception {
		...
		LERandomAccessFile ref = new LERandomAccessFile(indexPath, "r");
		int sqpackHeaderLength = checkSqPackHeader(ref);
		resloveSegments(ref, sqpackHeaderLength, segments);
		// Segment 1 is usually files, Segment 2/3 is unknown, Segment 4 is
		...
		if (segments[3] != null && segments[3].getOffset() != 0) {
			...
			for (int i = 0; i < numFolders; i++) {
				...
				indexMap.put(id, new SqPackIndexFolder(id, numFiles, fileIndexOffset));
				indexMap.get(id).readFiles(ref, false);
			}
		} else {
			int numFiles = (indexPath.contains("index2") ? 2 : 1) * segments[0].getSize() / 16;
			indexMap.put(0, new SqPackIndexFolder(0, numFiles, segments[0].getOffset()));
			indexMap.get(0).readFiles(ref, indexPath.contains("index2"));
		}
		// close index file
		ref.close();
		return indexMap;
	...
	public static int checkSqPackHeader(LERandomAccessFile ref) throws IOException {
		...
		if ((buffer[0] != 0x53) || (buffer[1] != 0x71) || (buffer[2] != 0x50) || (buffer[3] != 0x61)
				|| (buffer[4] != 0x63) || (buffer[5] != 0x6B)) {
			ref.close();
	...
	private static int resloveSegments(LERandomAccessFile ref, int segmentHeaderStart, SqPackDataSegment[] segments)
			throws IOException {
